...
import {
    ...
    getMilitaryEquipmentById,
    updateMilitaryEquipment,
...
function toDto(f: FormState): MilitaryEquipmentUpdateDto {
    return {
        name: f.name.trim(),
...
export default function MilitaryEquipmentForm() {
            ...
            .then(async (historiesData) => {
                ...
                if (isEdit) {
                    ...
                    setForm({
                        ...
                        historyId: eq.historyId ? String(eq.historyId) : "",
                    });
                }
            })
            .catch((err) => {
    ...
    async function onSubmit(e: FormEvent) {
        ...
        try {
            let targetId: number;
            if (isEdit) {
                await updateMilitaryEquipment(editId, toDto(form));
                targetId = editId;
            } else {
                const created = await createMilitaryEquipment({
                    ...toDto(form),
                    worldId: selectedWorld.id,
                });
                targetId = created.id;
            }
            navigate(`/storymap/military-equipment/${targetId}`);
        } catch (err) {
    ...
    async function onDelete() {
        ...
        setBusy(true);
        try {
            await deleteMilitaryEquipment(editId);
            navigate("/storymap/military-equipment");
        } catch (err) {
    ...
    return (
        <form className={s.page} onSubmit={onSubmit} noValidate>
            ...
            <div className={s.grid}>
                ...
                <div className={s.col}>
                    <OrnateField label={t("fields.equipmentType")}>
                        <OrnateTextInput
                            value={form.equipmentType}
                            maxLength={100}
                            placeholder={t("fields.equipmentTypeHint")}
                            onChange={(e) =>
            ...
            <div className={s.footer}>
                ...
                <Button
                    ...
                    onClick={() =>
                        navigate(
                            isEdit
                                ? `/storymap/military-equipment/${editId}`
                                : "/storymap/military-equipment"
                        )
                    }
                >
                    {t("form.cancel")}

...
import { HistoryDto, HobbyUpdateDto } from "../../../../interfaces/loreInterfaces";
...
function toDto(f: FormState): HobbyUpdateDto {
    return {
        name: f.name.trim(),
...
/** Zajednička forma za /hobbies/new i /:id/edit. */
export default function HobbyForm() {
    ...
    async function onSubmit(e: FormEvent) {
        ...
        try {
            let targetId: number;
            if (isEdit) {
                await updateHobby(editId, toDto(form));
                targetId = editId;
            } else {
                const created = await createHobby({
                    ...toDto(form),
                    worldId: selectedWorld.id,
                });
                targetId = created.id;
            }
            navigate(`/storymap/hobbies/${targetId}`);
        } catch (err) {
    ...
    async function onDelete() {
        ...
        setBusy(true);
        try {
            await deleteHobby(editId);
            navigate("/storymap/hobbies");
        } catch (err) {
        ...
        <form className={s.page} onSubmit={onSubmit} noValidate>
            ...
            <div className={s.grid}>
                <div className={s.col}>
                    ...
                    <OrnateField label={t("fields.activity")}>
                        <OrnateTextInput
                            value={form.activity}
                            maxLength={200}

...
import { LibraryDto } from "../../../../interfaces/loreInterfaces";
...
export default function LibraryList() {
    ...
    const newButton = canCreate ? (
        <Button onClick={() => navigate("/storymap/libraries/new")}>
            + {t("form.newTitle")}
        </Button>
    ) : undefined;
    ...
    const columns: Column<LibraryDto>[] = [
        ...
        },
        {
            ...
            value: (r) => (r.isPublic ? 1 : 0),
            render: (r) => <span>{r.isPublic ? "✓" : "—"}</span>,
        },
        {
            key: "updatedAt",
    ...
    return (
        <div className={s.page}>
            <DataTable<LibraryDto>
                data={libraries}
                columns={columns}
                getRowId={(r) => String(r.id)}
                onRowClick={(row) => navigate(`/storymap/libraries/${row.id}`)}
                title={t("listTitle")}

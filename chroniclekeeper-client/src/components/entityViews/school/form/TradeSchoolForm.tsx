...
/** Zajednička forma za /schools/new-trade i /schools/:id/edit-trade. */
export default function TradeSchoolForm() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            isEdit ? getTradeSchoolById(editId) : Promise.resolve(null),
        ])
            .then(([systems, school]) => {
                if (cancelled) return;
                ...
                if (school) {
                    setForm({
                        ...
                        durationYears:
                            school.durationYears != null
                                ? String(school.durationYears)
                                : "",
                        isGovernmentRecognized:
                            school.isGovernmentRecognized,
                    });
                }
            })
            .catch((err) => {
                console.error("Failed to load trade school form data:", err);
        ...
        try {
            ...
            const payload = {
                ...
                durationYears: form.durationYears.trim()
                    ? Number(form.durationYears)
                    : 0,
                isGovernmentRecognized: form.isGovernmentRecognized,
            };
            if (isEdit) {
                await updateTradeSchool(editId, payload);
                targetId = editId;
            } else {
                const created = await createTradeSchool({
                    ...payload,
            ...
            console.error("Failed to save trade school:", err);
    ...
    async function onDelete() {
        ...
        setBusy(true);
        try {
            await deleteTradeSchool(editId);
            navigate("/storymap/schools");
        } catch (err) {
            console.error("Failed to delete trade school:", err);
    ...
    return (
        <form className={s.page} onSubmit={onSubmit} noValidate>
            <h1 className={s.title}>
                {isEdit ? t("form.editTradeTitle") : t("form.newTradeTitle")}
            </h1>
            ...
            <div className={s.grid}>
                <div className={s.col}>
                    ...
                    <OrnateField label={t("fields.specialization")}>
                        <OrnateTextInput
                ...
                <div className={s.col}>
                    ...
                    <OrnateCheckbox
                        ...
                        checked={form.isGovernmentRecognized}
                        onChange={(e) =>
                            set("isGovernmentRecognized", e.target.checked)
                        }
                    />
                </div>
            </div>

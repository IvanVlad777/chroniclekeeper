...
import { TimelineDto } from "../../../../interfaces/loreInterfaces";
...
export default function TimelineList() {
    ...
    const newButton = canCreate ? (
        <Button onClick={() => navigate("/storymap/timelines/new")}>
            + {t("form.newTitle")}
        </Button>
    ) : undefined;

    const columns: Column<TimelineDto>[] = [
        {
            key: "name",
        ...
        },
        {
            ...
            render: (r) =>
                r.description ? (
                    <span className={s.description}>{r.description}</span>
                ) : (
                    "—"
                ),
        },
    ...
    return (
        <div className={s.page}>
            <DataTable<TimelineDto>
                data={timelines}
                columns={columns}
                getRowId={(r) => String(r.id)}
                onRowClick={(row) => navigate(`/storymap/timelines/${row.id}`)}
                title={t("listTitle")}

...
function toDto(f: FormState): AbilityUpdateDto {
    return {
        name: f.name.trim(),
...
/** Zajednička forma za /abilities/new i /abilities/:id/edit. */
export default function AbilityForm() {
    ...
    useEffect(() => {
        ...
        getAbilityById(editId)
            .then((a) => {
                if (cancelled) return;
                setForm({
                    name: a.name ?? "",
                    description: a.description ?? "",
                    type: (a.type as AbilityType) ?? "Physical",
                });
            })
            .catch((err) => {
    ...
    async function onSubmit(e: FormEvent) {
        ...
        try {
            let targetId: number;
            if (isEdit) {
                await updateAbility(editId, toDto(form));
                targetId = editId;
            } else {
            ...
            navigate(`/storymap/abilities/${targetId}`);
        } catch (err) {
    ...
    async function onDelete() {
        ...
        setBusy(true);
        try {
            await deleteAbility(editId);
            navigate("/storymap/abilities");
        } catch (err) {
                ...
                <div className={s.col}>
                    <OrnateField label={t("columns.type")} required>
                        <OrnateSelect
                            value={form.type}
                            onChange={(e) =>
                                set("type", e.target.value as AbilityType)
                            }
                        >

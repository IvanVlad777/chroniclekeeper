...
import { IndustryDto } from "../../../../interfaces/loreInterfaces";
...
export default function IndustryList() {
    ...
    const newButton = canCreate ? (
        <Button onClick={() => navigate("/storymap/industries/new")}>
            + {t("form.newTitle")}
        </Button>
    ) : undefined;

    const columns: Column<IndustryDto>[] = [
        {
            key: "name",
        ...
        {
            key: "employmentRate",
            header: t("columns.employmentRate"),
            sortable: true,
            align: "right",
            value: (r) => r.employmentRate,
            render: (r) => <span>{r.employmentRate}%</span>,
        },
        {
            key: "updatedAt",
    ...
    return (
        <div className={s.page}>
            <DataTable<IndustryDto>
                data={industries}
                columns={columns}
                getRowId={(r) => String(r.id)}
                onRowClick={(row) => navigate(`/storymap/industries/${row.id}`)}
                title={t("listTitle")}

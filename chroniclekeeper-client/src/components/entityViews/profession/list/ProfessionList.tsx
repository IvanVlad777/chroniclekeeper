...
import { ProfessionDto } from "../../../../interfaces/loreInterfaces";
...
export default function ProfessionList() {
    ...
    const newButton = canCreate ? (
        <Button onClick={() => navigate("/storymap/professions/new")}>
            + {t("form.newTitle")}
        </Button>
    ) : undefined;

    const columns: Column<ProfessionDto>[] = [
        {
            key: "name",
        ...
        {
            key: "workEnvironment",
            header: t("columns.workEnvironment"),
            sortable: true,
            value: (r) => r.workEnvironment ?? "",
            render: (r) => (
                <span className={s.workEnvironment}>
                    {r.workEnvironment || "—"}
                </span>
            ),
        },
    ...
    return (
        <div className={s.page}>
            <DataTable<ProfessionDto>
                data={professions}
                columns={columns}
                getRowId={(r) => String(r.id)}
                onRowClick={(row) => navigate(`/storymap/professions/${row.id}`)}
                title={t("listTitle")}

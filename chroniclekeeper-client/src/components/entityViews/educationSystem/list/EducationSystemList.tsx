...
export default function EducationSystemList() {
    ...
    const [systems, setSystems] = useState<EducationSystemDto[]>([]);
    ...
    const newButton = canCreate ? (
        <Button onClick={() => navigate("/storymap/education-systems/new")}>
            + {t("form.newTitle")}
        </Button>
    ) : undefined;

    const columns: Column<EducationSystemDto>[] = [
        {
            key: "name",
        ...
        {
            key: "isStateControlled",
            header: t("columns.stateControlled"),
            value: (r) => (r.isStateControlled ? 1 : 0),
            render: (r) => <span>{r.isStateControlled ? "✓" : "—"}</span>,
        },
        {
            key: "updatedAt",
    ...
    return (
        <div className={s.page}>
            <DataTable<EducationSystemDto>
                data={systems}
                columns={columns}
                getRowId={(r) => String(r.id)}
                onRowClick={(row) =>
                    navigate(`/storymap/education-systems/${row.id}`)
                }
                title={t("listTitle")}
